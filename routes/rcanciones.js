module.exports = function(app, swig, gestorBD) {
    ...
    app.post("/cancion", function(req, res) {
        ...
        gestorBD.insertarCancion(cancion, function(id){
            if (id == null) {
                showError("Error al insertar canción", res);
            } else {
                if (req.files.portada != null) {
                    var imagen = req.files.portada;
                    imagen.mv('public/portadas/' + id + '.png', function (err) {
                        if (err) {
                            showError("Error al subir la portada", res);
                        } else {
                    ...
                    if (req.files.audio != null) {
                        ...
                        audio.mv('public/audios/' + id + '.mp3', function (err) {
                            if (err) {
                                showError("Error al subir el audio", res);
                            } else {
                                res.redirect("/publicaciones");
                            }
                        });
    ...
    app.get('/cancion/:id', function (req, res) {
        ...
        gestorBD.obtenerCanciones(criterio,function(canciones){
            if ( canciones == null ){
                showError("Error al recuperar la canción", res);

            } else {
                let respuesta = swig.renderFile('views/bcancion.html',
                    {
                        cancion : canciones[0],
                        puedeReproducir: canciones[0].autor == req.session.usuario || comproCancion
                    });
                res.send(respuesta);
            }
        });
    ...
    app.post('/cancion/modificar/:id', function (req, res) {
        ...
        gestorBD.modificarCancion(criterio, cancion, function(result) {
            if (result == null) {
                showError("Error al modificar", res);
            } else {
                paso1ModificarPortada(req.files, id, function (result) {
                    if( result == null){
                        showError("Error en la modificación", res);
                    } else {
                        res.redirect("/publicaciones");
                    }
                });
    ...
    app.get('/cancion/comprar/:id', function (req, res) {
        ...
        gestorBD.obtenerCanciones(criterio, function (canciones) {
            if (canciones == null) {
                ...
                if (compra.usuario == canciones[0].autor) {
                    res.send("No puedes comprar tu propia canción!");
                }else if(comproCancion){
